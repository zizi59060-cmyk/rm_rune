...
public:
    SimGimbalSineTest()
        ...
        // 可调参数
        this->declare_parameter<double>("yaw_amp_deg", 90.0);      // yaw 正弦幅值，单位度
        this->declare_parameter<double>("pitch_amp_deg", 10.0);    // pitch 正弦幅值，单位度
        this->declare_parameter<double>("freq_hz", 0.10);          // 正弦频率
        this->declare_parameter<double>("pitch_phase_deg", 0.0);   // pitch 相位差
        this->declare_parameter<bool>("enable_pitch_sine", true);  // 是否让 pitch 也走正弦
        this->declare_parameter<double>("publish_rate_hz", 100.0); // 发送频率
...
private:
    void tf_callback(const tf2_msgs::msg::TFMessage::SharedPtr msg) {
        for (const auto &tf : msg->transforms) {
            ...
            // 与项目里现有姿态解法保持一致：yaw,pitch,roll = eulers(R, 2,1,0)
