...
public:
  ...
  // 新增：仿真运行时覆盖 camera->gimbal 外参
  void set_camera2gimbal(const Eigen::Matrix3d & R_camera2gimbal, const Eigen::Vector3d & t_camera2gimbal);
  ...
  cv::Point2f reproject_world_point(const Eigen::Vector3d & xyz_in_world) const {
      ...
      cv::projectPoints(pts_3d, cv::Vec3d(0,0,0), cv::Vec3d(0,0,0), camera_matrix_, distort_coeffs_, pts_2d);

      if (pts_2d.empty()) return {-1, -1};
      return pts_2d[0];
  }
...
private:
  ...
  const std::vector<cv::Point3f> OBJECT_POINTS = {
    cv::Point3f(0, 0, 0),
    cv::Point3f(0, -0.85,  0.16),
    cv::Point3f(0, -0.54,  0.18),
    cv::Point3f(0, -0.54, -0.18),
    cv::Point3f(0, -0.85, -0.16)
  };
};
