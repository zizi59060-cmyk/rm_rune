...
void MindVision::open()
{
  ...
  CameraSetFrameSpeed(handle_, 1);                         // 低帧率模式（可根据需要改为2/3）

  // // === 新增：防频闪控制 ===
  // // 启用自动防频闪，电网频率设置为50Hz（中国地区）
  // if (CameraSetAntiFlick(handle_, 1) == CAMERA_STATUS_SUCCESS &&
  //     CameraSetLightFrequency(handle_, 0) == CAMERA_STATUS_SUCCESS)
  // {
  //   tools::logger()->info("Anti-flicker enabled (Auto mode, 50Hz).");
  // }
  // else
  // {
  //   tools::logger()->warn("Failed to enable anti-flicker mode.");
  // }
  ...
  tools::logger()->info("Mindvision opened ({}x{}, exp={}ms, gamma={}).",
                        width_, height_, exposure_ms_, gamma_);
}
